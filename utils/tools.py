...
def adjust_learning_rate(optimizer, scheduler, epoch, args):
    if args.lradj == 'type1':
        ...
        lr_adjust = {epoch: args.learning_rate if epoch < 3 else args.learning_rate * (args.decay_fac ** ((epoch - 3) // 1))}
    ...
    # wandb.log({'lr': lr_adjust[epoch], "epoch": epoch})
...
def vali(model, vali_data, vali_loader, criterion, args, device, itr):
    ...
    if args.model == 'PatchTST' or args.model == 'DLinear' or args.model == 'TCN':
        ...
        for attr in ['gnn_layer', 'gnn_layer_l']:
            if hasattr(model.gpt2, attr):
                getattr(model.gpt2, attr).eval()
...
def test(model, test_data, test_loader, args, device, itr):
    ...
    mae, mse, rmse, mape, mspe, smape, nd = cal_metric(preds, trues)
...
def load_content(args):
    if 'ETT' in args.data:
        file = 'ETT'
    elif args.data in ["CY", "HD", "SJS"]:
        file = 'electricity_self'
    elif args.data in ['sl', 'wzp', 'yh']:
        file = 'pv_self'
    else:
        file = args.data
